...
# newMate=1 if the mate-terminal version >= 1.6
if [[ ("$(echo "$mateVersion" | cut -d"." -f1)" = "1" && \
      "$(echo "$mateVersion" | cut -d"." -f2)" -ge 6) || \
      "$(echo "$mateVersion" | cut -d"." -f1)" -ge 2 ]]
  then newMate="1"
  dconfdir=/org/mate/terminal/profiles
  echo DCONFDIR! woohoo! $dconfdir

else
  newMate=0
  gconfdir=/apps/mate-terminal/profiles
  echo "This script currently only tested on MATE >= 1.6"
  exit 1
fi
...
interactive_help() {
  ...
  echo -en "and which \nMATE Terminal profile to overwrite.\n"
  echo
